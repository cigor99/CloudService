...
public class VMService {
	...
	public Response addVM(VM vm) {
		...
		vm.setActivityList(activityList);
	...
	public Response editVM(VMWrapper vmWrap) {
		...
		if (!vmWrap.getOldName().equals(vmWrap.getName())) {
			if (vms.getVms().containsKey(vmWrap.getName()))
				return Response.status(400).entity("Error 400 : Virtual machine with given name already exists!")
						.build();
		}
		...
		if (curr.getRole().equals(Role.ADMIN)) {
			...
			if (!vmWrap.isChecked()) {
				if ((vm.getActivityList().get(vm.getActivityList().size() - 1).getOffTime()).equals("")) {
					vm.getActivityList().get(vm.getActivityList().size() - 1)
							.setOffTime(new SimpleDateFormat("dd.MM.yyyy HH:mm").format(new Date()));
				}
			}
		}
	...
	public HashMap<String, VM> filterVM(@FormParam("cpufrom") String cpufrom, @FormParam("cputo") String cputo,
			...
			@FormParam("gpufrom") String gpufrom, @FormParam("gputo") String gputo, @FormParam("text") String text) {
		...
		// ako je bilo sta od toga svega neko slovo tj nije prazan string ili broj
		// vracam gresku
		if (cpufromI == null || cputoI == null || ramfromI == null || ramtoI == null || gpufromI == null
				|| gputoI == null) {
			return null;
		}
		...
		for (VM vm : vms.values()) {
			if (vm.getNumCPUCores() >= cpufromI && vm.getNumCPUCores() <= cputoI && vm.getRamCapacity() >= ramfromI
					&& vm.getRamCapacity() <= ramtoI && vm.getNumGPUCores() >= gpufromI
					&& vm.getNumGPUCores() <= gputoI) {
